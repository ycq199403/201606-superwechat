...
public class ChatRoomAdapter extends ArrayAdapter<EMChatRoom> {
	...
	public ChatRoomAdapter(Context context, int res, List<EMChatRoom> groups) {
		...
		addChatRoomString = context.getResources().getString(cn.ucai.fulicenter.R.string.add_public_chat_room);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (getItemViewType(position) == 0) {
			...
			((TextView) convertView.findViewById(cn.ucai.fulicenter.R.id.name)).setText(addChatRoomString);
			...
			((TextView) convertView.findViewById(cn.ucai.fulicenter.R.id.name)).setText(getItem(position - 2).getName());
